...
const Feature108 = ({
  ...
  return (
    <section className="py-12 sm:py-20 bg-white dark:bg-gray-900/50 overflow-x-hidden">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex flex-col items-center gap-4 text-center">
          <Badge variant="outline" className="bg-secondary-300/25 border border-secondary-300">{badge}</Badge>
          <h1 className="max-w-2xl text-2xl font-semibold sm:text-3xl md:text-4xl text-secondary-200 dark:text-primary-100">
            {heading}
          </h1>
        ...
        <Tabs defaultValue={tabs[0].value} className="mt-8 w-full max-w-4xl mx-auto">
          <TabsList className="flex items-center justify-center gap-1 sm:gap-4 md:gap-10 grid w-full grid-cols-3 mb-6">
            {tabs.map((tab) => (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="flex items-center gap-1 sm:gap-2 rounded-xl px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-muted-foreground data-[state=active]:bg-muted data-[state=active]:text-lemon-200 data-[state=active]:dark:text-lemon-100"
              >
                {tab.icon} {tab.label}
              </TabsTrigger>
            ))}
          </TabsList>
          <div className="mx-auto mt-6 max-w-screen-xl rounded-2xl bg-muted/70 p-4 sm:p-6 lg:p-16">
            {tabs.map((tab) => (
              <TabsContent
                key={tab.value}
                value={tab.value}
                className="grid place-items-center gap-8 sm:gap-10 lg:grid-cols-2 lg:gap-10 mt-4"
              >
                <div className="flex flex-col gap-4 sm:gap-5 w-full">
                  <Badge variant="outline" className="w-fit bg-background">
                    {tab.content.badge}
                  </Badge>
                  <h3 className="text-2xl font-semibold sm:text-3xl lg:text-5xl text-gray-900 dark:text-gray-100">
                    {tab.content.title}
                  </h3>
                ...
                <div className="relative mx-auto w-full flex justify-center">
                  <div className="relative rounded-2xl overflow-hidden border-4 sm:border-8 border-gray-800 shadow-xl w-full max-w-[220px] sm:max-w-xs">
                    <img
